  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Event Recommendations</h1>
          <p className="text-slate-600">Pipeline-ready suggestions with attribution, segment fit, and projected impact.</p>
        </div>
      ...
      <div className="space-y-6">
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[...Array(2)].map((_, idx) => (
              <div key={idx} className="border border-slate-200 rounded-lg p-6 animate-pulse space-y-4">
                <div className="h-4 bg-slate-200 rounded w-2/3"></div>
                <div className="h-3 bg-slate-200 rounded w-1/2"></div>
                <div className="space-y-2">
                  {[...Array(3)].map((__, innerIdx) => (
                    <div key={innerIdx} className="h-3 bg-slate-200 rounded"></div>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="border border-dashed border-slate-300 rounded-lg p-10 text-center space-y-4 bg-white">
            <h3 className="text-lg font-semibold text-slate-900">No curated recommendations for this view</h3>
          ...
          filteredRecommendations.map((recommendation) => (
            <div key={recommendation.id} className="bg-white border border-slate-200 rounded-lg">
              <div className="border-b border-slate-200 p-6 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
              ...
              <div className="p-6 border-b border-slate-100 bg-slate-50 rounded-b-lg">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {recommendation.events.map((event, index) => (
                    <div key={event.id || index} className="border border-slate-200 rounded-lg p-4 space-y-3">
                      <div>
                        <h3 className="font-medium text-slate-900 line-clamp-2">{event.title}</h3>
                        <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                          <span className="px-2 py-0.5 rounded-full bg-slate-100">{event.accountTier || 'General'}</span>
