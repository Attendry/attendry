...
export class FirecrawlSearchService {
  ...
  static async searchEvents(params: FirecrawlSearchParams): Promise<FirecrawlSearchResult> {
    ...
    if (scrapeContent || extractSchema) {
      ...
      if (extractSchema) {
        // PHASE 3: Enhanced extraction prompt with speaker context and improved title extraction
        const defaultPrompt = extractPrompt || 
          "Extract event details from this page. CRITICAL: The title field is REQUIRED and must be extracted.\n\n" +
        ...
        console.log('[firecrawl-search-service] Using unified search+extract with enhanced title extraction prompt');
      }
    }
